...
export type Unshift<T extends TupleOf<unknown>, V = never> = Parameters<((head: V, ...args: T) => void)>;

export type SameLength<T> = {
    [K in keyof T]: unknown;
};

export type Zip<T extends TupleOf<unknown>, S extends SameLength<T>> = {
    [Index in keyof T]: [T[Index], S[Index]];
};
